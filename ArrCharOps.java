...
public class ArrCharOps {
    ...
    public static boolean equals(char[] arr1, char[] arr2) {
        ...
        //checks to see if the arrays length is different if so - not equal
        if (arr1.length != arr2.length){
            return false;
        }
        //since they are the same length - for loop that goes over both of them at the same time
    ...
    */
    public static char[] concat(char[] arr1, char[] arr2) {
        // check the length the new array i build needs to be
        int newlength = arr1.length + arr2.length;
        char[] newarr = new char [newlength];
        //loop that adds the first array to the new array
        for (int i=0; i < arr1.length; i++){
            newarr[i] = arr1 [i];
        }
    ...
    public static char[] subArray(char[] arr, int beginIndex, int endIndex) {
        // create a new array in the length i need
        char[] subarr = new char [endIndex-beginIndex];
        //loop that goes over the array between those indexes and adds them into the new array
        int subcount = 0;
        for (int i = beginIndex; i < endIndex; i++ ){
            subarr[subcount] = arr[i];
            subcount++ ;
        }
        return subarr;
    }
    ...
    public static long hashCode(char[] arr) {
        ...
        for (int i =0; i<arr.length; i++){
            ...
            hashcode += arr[i]*Math.pow(7.0, (double)(n-i-1));
        }
  ...
  //  Note that in this method you have to assume that both strings are not empty.
    public static int compareTo(String str1, String str2) {
        ...
        //once we know both strings are the same length we can start testing them
        if (str1.length() == str2.length()){

            for (int i = 0; i<str1.length(); i++){
                //if the cahrs at the same index in both strings check which is smaller
